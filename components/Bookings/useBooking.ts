...
const useBooking = (id: string) => {
  ...
  const transformedBookingResponse = bookingResponse?.data as
    | TransformedBookingData
    | undefined;
  const booking = transformedBookingResponse?.booking;

  const isWithin72HoursBeforeBooking =
    booking?.from &&
  ...
  const shouldShowDirections: boolean =
    booking?.status === 'cancelled' ||
    isWithin72HoursBeforeBooking ||
    hasBookingPassed ||
    false;
