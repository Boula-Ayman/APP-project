import { useGetCurrentUserProfileQuery, useUpdateUserProfileMutation } from '@/src/api/userApiSlice';
import { formatDateToISO, formatDateForDisplay } from '../../../utils/dateUtils';
...
export const useAccountForm = () => {
  const { data: userProfile, isLoading: isLoadingProfile, refetch } = useGetCurrentUserProfileQuery(undefined, {
  ...
  const verifyUpdate = async (updateData: any) => {
    ...
    if (updatedProfile) {
      ...
      if (verificationFailed.length > 0) {
        throw new Error(`Update verification failed for: ${verificationFailed.join(', ')}`);
      }
    }
  };
